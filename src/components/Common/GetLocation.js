...
export default {
    getWardByLocation(token, locationId) {
        const config = {
            headers: {
                token: token
            }
        }
        return axios.get("https://shipping.kiotapi.com/api/v3/ward/" + locationId, config)
    },
    getlocationcation(token) {
        const config = {
            token : token
        }
        return axios.get("https://shipping.kiotapi.com/api/v3/location", {headers: config})
    },
    getPrice(data, token) {
        const config = {
            token : token
